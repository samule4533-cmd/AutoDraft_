...
│    ├─ filter_by_threshold() distance ≤ 0.55 필터                   │
...
# 예시 로그:
# [req_id] threshold=0.55: 10개 검색 → 6개 통과, 4개 제거
```

| 거리 범위 | 의미 |
|-----------|------|
| 0.0 ~ 0.3 | 매우 유사 |
| 0.3 ~ 0.55 | 관련 있음 (현재 threshold) |
| 0.55 ~ 0.8 | 약한 관련, 차단 |
| 0.8 이상  | 무관, 차단 |

> threshold 0.55 근거: MiniLM 임베딩이 `【청구항 N】` 등 한국 특허 특수 표기를 0.50~0.55 구간에 분포시킴. 완전 무관한 질문은 0.65 이상으로 안전 마진 확보. 0.50은 과도하게 차단, 0.60은 노이즈 구간에 근접해 0.55로 결정.
...
| 증상 | 가능한 원인 | 진단 방법 | 해결 |
|------|------------|-----------|------|
| "사내 문서에 없습니다" (예상 외) | threshold가 너무 낮음 | `RagResult.top_distance` 확인 | `distance_threshold` 상향 (현재 0.55 → 0.6) |
